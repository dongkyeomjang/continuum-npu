...
- **bucket 격자 정렬이 gap 효과의 부호를 결정한다** ([TASK23](TASK23.md), 개입으로 확인). `padding_slots(N) = bucket_for(N) − N`이 있으면 AGENTIC이 유리하고 0이면 불리하다. 워크로드·seed·모델·slot 수를 고정하고 격자에 bucket 6만 추가하자 N=6 pooled ratio가 1.1504 → 0.9717로 내려가 역전이 소멸했다. **법칙의 형태는 `class`, 격자·문턱·크기는 `stack`**이다. 다만 **크기를 예측하는 모형은 아직 없다** — padding 비율 단조성이 N=5와 N=4에서 깨진다.
...
다음 권장 작업: (1) 격자 축의 **크기 모형** — bucket 점유 히스토그램에서 pooled ratio를 예측하는 모형. 이미 확보한 [TASK20](TASK20.md)·[TASK23](TASK23.md) 관측점으로 **새 측정 없이** 가능하다. (2) [TASK13](TASK13.md)의 후속 — 블록 반복과 bucket **전이** 상황 측정. (3) Stage 2 repeated-prefix baseline — 설계 제약은 [TASK11](TASK11.md)·[TASK14](TASK14.md). 전부 측정이 포함되므로 선등록 후 진행한다. 사용자 지시 없이 자동 착수하지 않는다.
...
- **[TASK23](TASK23.md)의 `INCONCLUSIVE` 3건(N=3, N=7, 개입 후 N=6)은 미결로 남긴다.** 셋 다 pooled와 방향은 분명하나 3블록 중 1블록이 동치 밴드에 걸렸다. 블록을 3 → 6으로 늘리면 해소될 가능성이 있고 재compile은 필요 없다.

## 다음 작업 후보

1. **격자 축의 크기 모형** — gap 분포가 만드는 bucket 점유 히스토그램에서 pooled ratio를 예측하는 모형을 세우고 [TASK20](TASK20.md)·[TASK23](TASK23.md) 관측점으로 검증한다. **새 측정이 필요 없다.**
2. [TASK23](TASK23.md)의 `INCONCLUSIVE` 3건 확정 — 블록을 3 → 6으로 늘린다. 재compile 불필요.
3. Stage 2 repeated-prefix baseline. APC OFF/ON을 독립 구성하며 설계 제약은 [TASK11](TASK11.md)의 "다음 작업" 절을 따른다. 선등록 후 진행한다.
